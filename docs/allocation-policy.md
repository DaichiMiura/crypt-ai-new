...
含み損益として計算する研究用モデルである。複数銘柄ポートフォリオはFundingを時刻順に
反映できるが、清算、取引所固有のinitial/maintenance margin、ADL、数量刻みは含まれない。
shortまたは追加レバレッジをpaperへ進める前に、それらを扱う実行・リスクモデルが必要である。
...
EXP-2026-0035以降の複数銘柄実験とEXP-2026-0042 paper/shadowは、この配分層を通した
同時ポートフォリオである。個別仮想口座を使う過去実験と混同せず、各検証報告に会計単位を
明記する。
