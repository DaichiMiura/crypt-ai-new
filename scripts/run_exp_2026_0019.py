#!/usr/bin/env python3
"""EXP-2026-0019の上限付き1,1,1,2ロット追加を比較する。"""
...
LOT_COUNTS_VARIANT = (1, 1, 1, 2)
MAX_TOTAL_LOT_COUNT_CONTROL = 4
MAX_TOTAL_LOT_COUNT_VARIANT = 5


def main() -> None:
    """固定ロット版と1,1,1,2ロット版を6銘柄で比較し成果物を保存する。"""
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("artifacts/EXP-2026-0019")
    )
    ...
    for symbol in sorted(VOID_SHORT_SYMBOLS):
        ...
        for name, result in (("control", control), ("fib-lots-1_1_1_2", variant)):
    ...
    payload = {
        "experiment_id": "EXP-2026-0019",
        "status": "BACKTEST_COMPLETED",
        "control": "entry_lot_counts=1,1,1,1; max_total_lot_count=4",
        "variant": "entry_lot_counts=1,1,1,2; max_total_lot_count=5",
        ...
        "limitations": [
            ...
            "variantは4つの登録済み水準へ1,1,1,2ロットを割り当て、未約定水準の再配分を行わない。",
